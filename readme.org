...
#+begin_src emacs-lisp
  (defvar-keymap my-command-map
    ...
    "y" '(menu-item "Copy files to clipboard"
                    my/dired-copy-files-to-clipboard)
    "r" '(menu-item "Emacs resource status"
                    my/emacs-resource-status))
...
*** key bindings
普段よく使う出力操作にショートカットを割り当てる。
e = export のつもりでまとめている。

#+begin_src emacs-lisp
  (global-set-key (kbd "C-c e p") #'my/pandoc-org-to-pdf)
  (global-set-key (kbd "C-c e d") #'my/pandoc-org-to-docx)
  (global-set-key (kbd "C-c e m") #'my/pandoc-org-to-markdown)
  (global-set-key (kbd "C-c e t") #'my/pandoc-org-to-txt)
  (global-set-key (kbd "C-c e b") #'my/pandoc-buffer-org-to-pdf)
#+end_src
...
#+begin_src shell
  ...
  export EMACSCLIENT_PATH="/ucrt64/bin/emacsclientw.exe"
  emacs-vterm() {
      local dir="${1:-$PWD}"
