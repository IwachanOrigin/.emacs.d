...
*** which-key-postframe
whick-keyの表示をウィンドウではなくポストフレーム(子フレーム)として表示する機能です.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  ;; vtermで開いているフォルダをdirvishで開き、UIで操作する
  (defun my/vterm-open-pwd-in-dirvish ()
    "Open vterm's current directory in full Dirvish."
    (interactive)
...
** eglot config
*** diff-hl の再起動
eglogがdiff-hlの後に起動すると左マージンのgit差分が出ないので再起動する  
またflymakeとのぶつかり合いも考え、こちらも再起動する
#+begin_src emacs-lisp
  (defun my/restart-diff-hl-buffer (buf)
    "Restart diff-hl margin in BUF."
  ...
  (defun my/restart-diff-hl-after-eglot-connect (_server)
    "Restart diff-hl after Eglot server connection is established."
    ;; Eglot/Flymake の初期反映が少し遅れる場合に備えて遅延
    (run-at-time
     0.5 nil
     (lambda ()
       (dolist (buf (buffer-list))
         (with-current-buffer buf
           (when (and (fboundp 'eglot-managed-p)
                      (eglot-managed-p))
             (my/restart-diff-hl-buffer buf)))))))

  (defun my/restart-diff-hl-after-flymake ()
    "Restart diff-hl after Flymake starts in Eglot buffers."
    (when (and (fboundp 'eglot-managed-p)
               (eglot-managed-p))
  ...
  (with-eval-after-load 'flymake
    (add-hook 'flymake-mode-hook #'my/restart-diff-hl-after-flymake))
#+end_src
...
#+begin_src emacs-lisp
  (progn
    ...
    (with-eval-after-load 'eglot
      ...
      ;; Eglot 起動後に diff-hl の margin 表示を復元
      (add-hook 'eglot-connect-hook #'my/restart-diff-hl-after-eglot-connect))
...
#+begin_src emacs-lisp
  (use-package flutter
    ...
    :bind (:map dart-mode-map
                ...
                ("C-c C-a" . flutter-run)
                ("C-c C-t" . flutter-test-current-file)))
#+end_src
