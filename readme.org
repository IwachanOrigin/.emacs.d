...
#+begin_verse
イカれた問題であるgpgのフォルダパス問題を解決するために作成した酷い関数です。
msys2を使っていると、gpgのエラーが起き、パッケージに更新が出来ません。署名が認証できないからです。
gpgを使って手動で署名を発行するか、gnu-elpa-keyring-update(?)のようなものをインストールする必要があります。
ただ、msys2を使っていると「expand-file-name」が「C:\」のパスで解決してしまいます。
結果、認証時に「/C:/.../elpa/gnupg」のフォルダを見に行くのですが、パスの解決が出来ないので認証エラーとなります。
それを解決するため、C:/だったら/c/に変換する関数を書きました。
#+end_verse
#+begin_src emacs-lisp
  (defun my-windows-path-to-msys (path)
    "Windows形式 C:/foo/bar を MSYS2形式 /c/foo/bar に変換する。すでに /c/... 形式ならそのまま返す。"
    (cond
     ((null path) nil)
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (setq package-gnupghome-dir
          (my-windows-path-to-msys (expand-file-name "elpa/gnupg" user-emacs-directory)))
...
#+begin_src emacs-lisp
  ...
  (defhydra hydra-shortcut-of-emacs (:hint nil
   ...
   _M->_: バッファの末尾へ移動    _C-x SPC_: C-o > 空白挿入_C-x r_: emacs restart
